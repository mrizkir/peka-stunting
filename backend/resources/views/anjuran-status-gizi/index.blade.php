<x-layouts.app
  title="Anjuran Status Gizi"
  eyebrow="Bayi dan Balita"
  heading="Anjuran Status Gizi (Z-Score)"
  description="Kelola aturan anjuran hasil skrining z-score per indikator dan kelompok kebutuhan."
>
  @if (session('success'))
    <div class="alert alert-success mb-6 text-sm">{{ session('success') }}</div>
  @endif

  <x-ui.card title="Kelompok kebutuhan" description="Aturan anjuran untuk TB/U, BB/U, BB/TB, dan anjuran utama.">
          ...
          @forelse ($entries as $entry)
            <tr class="border-b border-base-200 last:border-0">
              ...
              <td class="px-4 py-3 text-right">
                <a
                  href="{{ route('anjuran-status-gizi.edit', $entry['menu_slug']) }}"
          ...
          @empty
            <tr>
              <td colspan="5" class="px-4 py-8 text-center text-base-content/60">
                Belum ada konten Periksa Status Gizi. Jalankan seeder edukasi terlebih dahulu.
              </td>
            </tr>
          @endforelse
        </tbody>
