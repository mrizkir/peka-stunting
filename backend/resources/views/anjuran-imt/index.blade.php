<x-layouts.app
  ...
  heading="Anjuran IMT"
  description="Kelola aturan anjuran hasil skrining IMT per kelompok kebutuhan (menu edukasi)."
>
  @if (session('success'))
    <div class="alert alert-success mb-6 text-sm">{{ session('success') }}</div>
  @endif

  <x-ui.card title="Kelompok kebutuhan" description="Setiap menu yang memiliki kalkulator Cek IMT dapat memiliki anjuran berbeda.">
          ...
          @forelse ($entries as $entry)
            <tr class="border-b border-base-200 last:border-0">
              ...
              <td class="px-4 py-3 text-right">
                <a
                  href="{{ route('anjuran-imt.edit', $entry['menu_slug']) }}"
          ...
          @empty
            <tr>
              <td colspan="5" class="px-4 py-8 text-center text-base-content/60">
                Belum ada konten Cek IMT. Jalankan seeder edukasi terlebih dahulu.
              </td>
            </tr>
          @endforelse
        </tbody>
