<x-layouts.app
  title="Anjuran Cek Keberhasilan Menyusui"
  eyebrow="Ibu Nifas dan Menyusui"
  heading="Anjuran Cek Keberhasilan Menyusui"
  description="Kelola aturan anjuran hasil skrining keberhasilan menyusui per kelompok kebutuhan (menu edukasi)."
>
  @if (session('success'))
    <div class="alert alert-success mb-6 text-sm">{{ session('success') }}</div>
  @endif

  <x-ui.card title="Kelompok kebutuhan" description="Setiap menu yang memiliki kalkulator Cek Keberhasilan Menyusui dapat memiliki anjuran berbeda.">
          ...
          @forelse ($entries as $entry)
            <tr class="border-b border-base-200 last:border-0">
              ...
              <td class="px-4 py-3 text-right">
                <a
                  href="{{ route('anjuran-menyusui.edit', $entry['menu_slug']) }}"
          ...
          @empty
            <tr>
              <td colspan="5" class="px-4 py-8 text-center text-base-content/60">
                Belum ada konten Cek Keberhasilan Menyusui. Jalankan seeder edukasi terlebih dahulu.
              </td>
            </tr>
          @endforelse
        </tbody>
